...
public class BattleShipsAI implements Serializable {
     ...
     * this point.
     */
    private ArrayList<int[]> lastStrikeList;
    ...
    public BattleShipsAI(IBoard myBoard, IBoard opponentBoard) {
        ...
        size = board.getSize();
        lastStrikeList=new ArrayList<int[]>();
    }
    ...
    public void putShips(AbstractShip ships[]) {
        ...
        for (AbstractShip s : ships) {
            do {
                ...
                x=rnd.nextInt(size);
                y=rnd.nextInt(size);
                int i=rnd.nextInt(orientations.length);
                o=orientations[i];
                s.setNavireOri(o);
    ...
    public Hit sendHit(int[] coords) {
        ...
        if (lastVertical != null) {
            ...
            while(lastStrikeList.size()>0){
                lastStrike=lastStrikeList.get(lastStrikeList.size()-1);
                ...
                if (res == null) {
                    // no suitable coord found... forget last strike.
                    lastStrikeList.remove(lastStrikeList.size()-1);
                    lastStrike = null;
                }
                else{
                    break;
    ...
    private boolean canPutShip(AbstractShip ship, int x, int y) {
        Orientation o = ship.getNavireOri();
        int dx = 0, dy = 0;
        if (o == Orientation.EAST) {
            if (x + ship.getNavireLength() >= this.size) {
                return false;
            }
            dx = 1;
        } else if (o == Orientation.SOUTH) {
            if (y + ship.getNavireLength() >= this.size) {
                return false;
            }
            dy = 1;
        } else if (o == Orientation.NORTH) {
            if (y + 1 - ship.getNavireLength() < 0) {
                return false;
            }
            dy = -1;
        } else if (o == Orientation.WEST) {
            if (x + 1 - ship.getNavireLength() < 0) {
                return false;
            }
            dx = -1;
        }
        ...
        for (int i = 0; i < ship.getNavireLength(); ++i) {
    ...
    private boolean isUndiscovered(int x, int y) {
        return x >= 0 && x < size && y >= 0 && y < size && !board.getHit(x, y);
    }
